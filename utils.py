...
def check_scene(env, state, start_rot, object_performance=None, scene_name=None, 
    ...
    通过距离、视图、手方向、目标对象状态和对象计数检查场景是否有效
    """
    MAX_TEST_PER_OBJ = 10
...
def mkdir_if_missing(dst_dir):
    """ 用于文件不存在时,创建文件 """
    if not os.path.exists(dst_dir):  # 如果括号里的文件不存在
        os.makedirs(dst_dir)  # 递归创建该目录
...
def regularize_pc_point_count(pc, npoints, use_farthest_point=False):
    ...
    如果点云pc中的点少于npoints,会出现过采样
    否则,他会对输入的点云pc进行降采样以获得npoints个点
    使用最远点采样方法：指示是否使用最远点采样
    减少取样点,使用最远点采样的版本运行较慢
    """
